...
export function PromotionUsageChart() {
  ...
  const barOption = {
    tooltip: {
      ...
      formatter: (params: { dataIndex: number; value: number }[]) => {
        const p = params[0]
        const pr = sorted[p.dataIndex]
        const statusColor = STATUS_COLORS[pr?.status ?? 'active']
        return `<p style="font-weight:700;font-size:12px;color:#0F172A;margin-bottom:6px">${pr?.promo_name}</p>
          ...
          <p style="font-size:11px;margin-top:4px">Status: <b style="color:${statusColor}">${pr?.status}</b></p>`
      },
    },
    ...
    series: [{
      ...
      label: {
        ...
        formatter: (p: { value: unknown }) =>
          view === 'usage' ? `${formatNumber(Number(p.value))}×`
  ...
  const trendOption = {
    ...
    xAxis: {
      type: 'category',
      data: trend.map((t) => formatDate(t.month, 'MMM yy')),
      boundaryGap: false,
    ...
    yAxis: [
      {
        type: 'value',
        axisLabel: { color: '#94A3B8', fontSize: 10, formatter: (v: number) => `${(v / 1_000_000).toFixed(0)}jt` },
        splitLine: { lineStyle: { color: '#F1F5F9', type: 'dashed' } },
        axisLine: { show: false }, axisTick: { show: false },
      },
      {
    ...
    series: [
      {
        ...
        lineStyle: { color: '#37B220', width: 2.5 },
        itemStyle: { color: '#37B220' },
        areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: 'rgba(55,178,32,0.2)' }, { offset: 1, color: 'rgba(55,178,32,0.01)' }] } },
      },
      {
        name: 'Diskon',
        type: 'line', smooth: true, showSymbol: false,
        data: trend.map((t) => t.discount),
      ...
      },
      {
        ...
        itemStyle: { color: '#0878C820', borderRadius: [4, 4, 0, 0] },
        barMaxWidth: 24,
      },
    ],
  }
    ...
    <div className="rounded-2xl border border-[#E2E8F0] bg-white">
      <div className="flex flex-wrap items-start justify-between gap-2 px-5 pt-5 pb-4 border-b border-[#F1F5F9]">
        <div>
          ...
          <p className="text-xs text-[#64748B] mt-0.5">{promos.length} promosi aktif/berakhir</p>
        </div>
