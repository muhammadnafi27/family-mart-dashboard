  ...
  return (
    <tr className="border-b border-[#F1F5F9]">
      {[120, 180, 80, 72, 72, 72, 80, 64].map((w, i) => (
        <td key={i} className="px-3 py-3"><div className="h-3.5 rounded-md bg-slate-100 animate-pulse" style={{ width: w }} /></td>
      ))}
    </tr>
  )
}
    ...
    <div className="rounded-2xl border border-[#E2E8F0] bg-white overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-[#F1F5F9]">
        <div>
          ...
          <p className="text-xs text-[#64748B] mt-0.5">{!isLoading && `${filtered.length} produk dari ${total.toLocaleString('id-ID')} total`}</p>
        </div>
        <div className="relative">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-[#94A3B8]" />
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari promo / produk..."
        ...
        <table className="w-full text-sm min-w-[860px]">
          <thead>
            <tr className="border-b border-[#F1F5F9] bg-[#F8FAFC]">
              {['Promosi','Produk','Kategori','Tipe','Nilai','Digunakan','Diskon Total','Revenue','Status'].map((h) => (
          ...
          <tbody>
            {isLoading ? Array.from({ length: 8 }).map((_, i) => <RowSkeleton key={i} />)
              : isError ? <tr><td colSpan={9} className="py-12 text-center text-sm text-[#64748B]">Gagal memuat data</td></tr>
              : filtered.length === 0 ? (
                ...
                return (
                  <tr key={i} className="border-b border-[#F1F5F9] hover:bg-[#F8FAFC] transition-colors">
                    ...
                    <td className="px-3 py-2.5"><p className="text-xs font-medium text-[#0F172A] max-w-[160px] truncate">{r.product_name}</p></td>
                    <td className="px-3 py-2.5"><span className="text-[11px] text-[#64748B]">{r.category_name}</span></td>
                    <td className="px-3 py-2.5">
                      <span className={`text-[11px] font-semibold rounded-full px-2 py-0.5 ${r.discount_type === 'percent' ? 'bg-[#EFF6FF] text-[#0878C8]' : 'bg-[#FFF7ED] text-[#C2410C]'}`}>
                        {r.discount_type === 'percent' ? '%' : 'Rp'}
                      </span>
                    </td>
                    <td className="px-3 py-2.5">
                      <span className="text-xs font-semibold text-[#F97316]">{r.discount_type === 'percent' ? `${r.discount_value}%` : formatCurrency(r.discount_value)}</span>
                    </td>
                    <td className="px-3 py-2.5">
                    ...
                    <td className="px-3 py-2.5"><span className="text-xs font-medium text-[#F97316]">{r.total_discount > 0 ? formatCurrency(r.total_discount) : '—'}</span></td>
                    <td className="px-3 py-2.5"><span className="text-xs font-semibold text-[#0F172A]">{r.revenue > 0 ? formatCurrency(r.revenue) : '—'}</span></td>
                    <td className="px-3 py-2.5">
                      <span className="text-[11px] font-bold rounded-full px-2.5 py-0.5" style={{ background: st.bg, color: st.color }}>{st.label}</span>
                    </td>
                  </tr>
                )
              })}
