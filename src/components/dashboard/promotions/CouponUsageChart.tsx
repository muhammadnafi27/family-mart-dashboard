...
export function CouponUsageChart() {
  ...
  const donutOption = {
    ...
    series: [{
      type: 'pie', radius: ['52%', '75%'], center: ['50%', '50%'], padAngle: 3,
      data: [
      ...
      label: { show: false },
      emphasis: { label: { show: true, fontSize: 13, fontWeight: 700, formatter: '{d}%', color: '#0F172A' }, itemStyle: { shadowBlur: 10, shadowColor: 'rgba(0,0,0,0.15)' } },
    }],
  }
  ...
  const barOption = {
    tooltip: {
      ...
      formatter: (params: { dataIndex: number; value: number }[]) => {
        const p = params[0]; const c = sorted[p.dataIndex]
    ...
    xAxis: { type: 'value', max: 100, axisLabel: { color: '#94A3B8', fontSize: 10, formatter: (v: number) => `${v}%` }, splitLine: { lineStyle: { color: '#F1F5F9', type: 'dashed' } }, axisLine: { show: false }, axisTick: { show: false } },
    yAxis: { type: 'category', data: sorted.map((c) => c.promo_name.length > 20 ? c.promo_name.slice(0, 20) + '…' : c.promo_name), axisLabel: { color: '#64748B', fontSize: 10.5 }, axisLine: { show: false }, axisTick: { show: false } },
    series: [{
      type: 'bar',
      data: sorted.map((c) => ({ value: c.usage_rate, itemStyle: { color: c.usage_rate >= 70 ? '#37B220' : c.usage_rate >= 40 ? '#0878C8' : '#F97316', borderRadius: [0, 6, 6, 0] } })),
      barWidth: 16,
      label: { show: true, position: 'right', formatter: (p: { value: unknown }) => `${Number(p.value).toFixed(1)}%`, color: '#64748B', fontSize: 10, fontWeight: 600 },
    }],
  }
  ...
  return (
    <div className="rounded-2xl border border-[#E2E8F0] bg-white h-full">
      <div className="px-5 pt-5 pb-4 border-b border-[#F1F5F9]">
        ...
        <p className="text-xs text-[#64748B] mt-0.5">{!isLoading && `${formatNumber(totalUsed)} / ${formatNumber(totalIssued)} kupon digunakan`}</p>
      </div>
      <div className="p-5">
        {isLoading ? <div className="h-64 rounded-xl bg-slate-100 animate-pulse" /> :
          isError || coupons.length === 0 ? <div className="h-64 flex items-center justify-center text-sm text-[#64748B]">{isError ? 'Gagal memuat' : 'Tidak ada kupon'}</div> :
