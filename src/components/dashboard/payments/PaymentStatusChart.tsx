    ...
    series: [{
      ...
      data: statuses.map((s) => {
        ...
        return {
          name: s.status,
          value: s.count,
          itemStyle: { color: cfg?.color ?? '#94A3B8', borderRadius: 6, borderColor: '#fff', borderWidth: 2 },
        }
      }),
      label: { show: false },
    ...
    <div className="rounded-2xl border border-[#E2E8F0] bg-white h-full">
      ...
      <div className="p-5">
        {isLoading
          ...
          : statuses.length === 0
            ? <div className="h-48 flex items-center justify-center text-sm text-[#64748B]">Tidak ada data</div>
            : (
              <>
...
/* ── Transaction table (moved here for colocation) ── */
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-[#F1F5F9]">
        <div>
          ...
          <p className="text-xs text-[#64748B] mt-0.5">
            {!isLoading && total > 0 && `${((page - 1) * pageSize + 1).toLocaleString('id-ID')}–${Math.min(page * pageSize, total).toLocaleString('id-ID')} dari ${total.toLocaleString('id-ID')}`}
          </p>
        </div>
                ...
                <tr key={i} className="border-b border-[#F1F5F9]">
                  {COLS.map((_, j) => (
                    <td key={j} className="px-3 py-3">
                      <div className="h-3.5 rounded-md bg-slate-100 animate-pulse" style={{ width: j === 0 ? 88 : 64 }} />
                    </td>
                  ))}
                </tr>
              ))
              : isError
                ? <tr><td colSpan={COLS.length} className="py-12 text-center text-sm text-[#64748B]">Gagal memuat data</td></tr>
                : rows.length === 0
                  ? <tr><td colSpan={COLS.length} className="py-12 text-center"><div className="flex flex-col items-center gap-2 text-[#64748B]"><span className="text-2xl">💳</span><p className="text-sm font-medium">Tidak ada transaksi ditemukan</p></div></td></tr>
                  : rows.map((r) => {
                    ...
                    return (
                      <tr key={r.payment_id} className="border-b border-[#F1F5F9] hover:bg-[#F8FAFC] transition-colors">
                        <td className="px-3 py-2.5">
                          <span className="font-mono text-[11px] font-bold text-[#0878C8]">{r.invoice_number}</span>
                        </td>
                        <td className="px-3 py-2.5">
                          <span className="text-[11px] text-[#64748B] max-w-[100px] truncate block">{r.store_name}</span>
                        </td>
                        <td className="px-3 py-2.5">
                          {r.customer_name
                            ? <span className="text-xs text-[#0F172A]">{r.customer_name}</span>
                        ...
                        <td className="px-3 py-2.5">
                          <span className="text-sm font-bold text-[#0F172A]">{formatCurrency(r.paid_amount)}</span>
                        </td>
                        <td className="px-3 py-2.5">
                          <span className="text-[11px] text-[#64748B] whitespace-nowrap">{formatDateTime(r.payment_time)}</span>
                        </td>
                        <td className="px-3 py-2.5">
                          <span className="font-mono text-[11px] text-[#94A3B8]">{r.reference_no ?? '—'}</span>
                        </td>
                        <td className="px-3 py-2.5">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-t border-[#F1F5F9] bg-[#F8FAFC]">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => setPage(page - 1)} disabled={page <= 1}
            className="h-7 w-7 flex items-center justify-center rounded-md border border-[#E2E8F0] bg-white disabled:opacity-40 hover:border-[#0878C8]">
