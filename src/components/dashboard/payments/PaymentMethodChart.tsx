...
export function PaymentMethodChart() {
  ...
  const donutOption = {
    ...
    legend: {
      ...
      textStyle: { color: '#64748B', fontSize: 11 }, icon: 'circle', itemWidth: 8, itemHeight: 8,
    },
  }
      ...
      <div className="p-5">
        {isLoading
          ...
          : isError || methods.length === 0
            ? <div className="h-52 flex items-center justify-center text-sm text-[#64748B]">{isError ? 'Gagal' : 'Tidak ada data'}</div>
            : (
              <>
                ...
                <div className="mt-4 space-y-2">
                  {methods.map((m, i) => (
                    <div key={m.method_name} className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold rounded-full px-1.5 py-0.5 min-w-[38px] text-center"
                        style={{ background: `${getColor(m.method_name, i)}15`, color: getColor(m.method_name, i) }}>
                        {m.percent.toFixed(1)}%
                      </span>
                    </div>
                  ))}
